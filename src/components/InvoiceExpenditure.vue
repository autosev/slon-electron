...
const { createEditor, openDraft } = invoiceExpenditureStore
const { selectedTab, editorData, drafts } = storeToRefs(invoiceExpenditureStore)
...
onActivated(() => {
  if (editorData.value.invoice.sender_stock_id !== null) {
    useTableStore().setSelectedStockId(editorData.value.invoice.sender_stock_id)
  }
})
</script>
